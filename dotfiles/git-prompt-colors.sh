override_git_prompt_colors() {
  ...
  GIT_PROMPT_PREFIX="${NF_BRANCH} "                 # start of the git info string
  GIT_PROMPT_SUFFIX=""                 # the end of the git info string
  ...
  GIT_PROMPT_CONFLICTS="${Red}${NF_X} "       # the number of files in conflict
  # GIT_PROMPT_CHANGED="${Blue}✚ "        # the number of changed files
   GIT_PROMPT_CHANGED="${Blue}${NF_PLUS} "        # the number of changed files

  # GIT_PROMPT_REMOTE=" "                 # the remote branch name (if any) and the symbols for ahead and behind
  # GIT_PROMPT_UNTRACKED="${Cyan}…"       # the number of untracked files/dirs
  # GIT_PROMPT_STASHED="${BoldBlue}⚑ "    # the number of stashed files/dir
  # GIT_PROMPT_CLEAN="${BoldGreen}✔"      # a colored flag indicating a "clean" repo
  GIT_PROMPT_STASHED="${BoldBlue}${NF_FLAG} "    # the number of stashed files/dir
  ...
  GIT_PROMPT_SYMBOLS_NO_REMOTE_TRACKING="${NF_COMMIT_LOCAL}" # This symbol is written after the branch, if the branch is not tracked 
}
